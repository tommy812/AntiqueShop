...
const Contact = () => {
  ...
  const [errorMessage, setErrorMessage] = useState('Please fix the errors in the form and try again.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const touchedFields = Object.keys(formState).reduce((acc, field) => {
      acc[field] = true;
      return acc;
  ...
  const contactInfo = {
    address: settings?.address ? `${settings.address.street || ''}, ${settings.address.postalCode || ''} ${settings.address.city || ''}, ${settings.address.country || ''}` : 'Via dei Fossi, 50, 50123 Florence, Italy',
